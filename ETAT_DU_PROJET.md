...
- `index.html` : application web autonome « Réseau PERT », composée d’un seul fichier HTML avec CSS et JavaScript intégrés (4 784 lignes, 172 767 octets après les huit rustines de l’audit).
...
- Première rustine de l’audit appliquée localement le 26 août 2026 : les touches Suppr et Retour arrière demandent désormais la même confirmation intégrée que le bouton avant de supprimer une liaison. Elles restent sans effet sur la liaison lorsque l’événement provient d’un champ de saisie, d’une zone de texte, d’un menu ou d’un contenu éditable. Le mode d’emploi a été actualisé. Test ciblé réussi dans le navigateur : chaque touche ouvre la confirmation, l’annulation conserve les 6 liaisons et Retour arrière dans un champ de date ne déclenche aucune confirmation ni suppression. Syntaxe JavaScript et différences Git contrôlées. Cette correction n’est pas publiée.
- Deuxième rustine appliquée localement le 26 août 2026 : la suppression d’une ressource non affectée utilise désormais la même confirmation intégrée que les activités et les liaisons, à la place de `window.confirm()`. Le refus de supprimer une ressource affectée reste inchangé. Test ciblé réussi dans le navigateur avec une ressource temporaire : message nominatif affiché dans la fenêtre intégrée, aucune confirmation native et annulation sans suppression. Syntaxe JavaScript et différences Git contrôlées. Cette correction n’est pas publiée.
- Troisième rustine appliquée localement le 26 août 2026 : le bouton « Supprimer » de la fenêtre Ressources utilise désormais aussi le déclenchement direct sur `pointerup` pour les pointeurs tactiles, déjà employé par les autres suppressions, tout en conservant `click` sur ordinateur et son état `disabled`. Test ciblé réussi dans le navigateur : un clic ouvre une seule confirmation et l’annulation conserve la ressource temporaire. Le branchement tactile unique, la syntaxe JavaScript et les différences Git sont contrôlés. Cette correction n’est pas publiée ; un essai réel sur iPad reste recommandé.
- Quatrième rustine appliquée localement le 26 août 2026 : le bouton « Retirer » d’une affectation de ressource utilise désormais aussi le déclenchement direct sur `pointerup` pour les pointeurs tactiles, tout en conservant `click` sur ordinateur et son état `disabled`. Test ciblé réussi dans le navigateur : sélection d’une affectation, retrait provisoire faisant passer la liste de 3 à 2 éléments, puis annulation de la fiche et restauration des 3 affectations à sa réouverture. Le branchement tactile unique, la syntaxe JavaScript et les différences Git sont contrôlés. Cette correction n’est pas publiée ; un essai réel sur iPad reste recommandé.
- Cinquième rustine appliquée localement le 26 août 2026 : le bouton dynamique « Retirer » d’une date non travaillée utilise désormais aussi le déclenchement direct sur `pointerup` pour les pointeurs tactiles, avec une action unique partagée avec `click`. Test ciblé réussi dans le navigateur : ajout provisoire du 15 octobre 2026, retrait ramenant la liste à l’état vide, puis annulation et confirmation à la réouverture que le calendrier enregistré reste inchangé. La syntaxe JavaScript et les différences Git sont contrôlées. Cette correction n’est pas publiée ; un essai réel sur iPad reste recommandé.
- Sixième rustine appliquée localement le 26 août 2026 : « Nouveau projet » remplace désormais sa confirmation native par la fenêtre intégrée avant tout remplacement d’un planning non vide. Le titre devient « Confirmer le nouveau projet » et le bouton « Créer » ; les confirmations de suppression reprennent automatiquement leurs libellés habituels. Test ciblé réussi dans le navigateur sur 5 activités et 6 liaisons : aucune confirmation native, annulation sans perte, puis contrôle du retour des libellés de suppression. La syntaxe JavaScript et les différences Git sont contrôlées. Cette correction n’est pas publiée.
- Septième rustine appliquée localement le 26 août 2026 : « Ouvrir » remplace désormais sa confirmation native par la fenêtre intégrée lorsqu’un planning est déjà affiché. Le fichier reste lu et validé avant la demande ; le titre devient « Confirmer l’ouverture » et le bouton « Ouvrir ». L’application asynchrone du projet validé est isolée et ses erreurs restent gérées après confirmation. Syntaxe JavaScript, absence de confirmation native dans ce flux, capture du projet et réinitialisation du sélecteur contrôlées. Le sélecteur de fichier du navigateur intégré n’ayant pas pu être automatisé, un essai fonctionnel réel sur iPad reste nécessaire. Cette correction n’est pas publiée.
- Huitième rustine appliquée localement le 26 août 2026 : « Lier le fichier de travail » remplace désormais sa confirmation native par la fenêtre intégrée lorsqu’un planning est déjà affiché. Le fichier reste sélectionné, lu et validé avant la demande ; le titre devient « Confirmer la liaison » et le bouton « Lier ». L’application asynchrone du projet et de la liaison au fichier est isolée et ses erreurs restent gérées après confirmation. Syntaxe JavaScript, absence de confirmation native dans ce flux, capture du projet et du fichier et établissement différé de la liaison contrôlés. L’API système `showOpenFilePicker` n’étant pas disponible dans le navigateur de test, un essai fonctionnel réel dans un navigateur compatible reste nécessaire. Cette correction n’est pas publiée. La seule confirmation native restante est l’avertissement non destructif précédant le téléchargement d’une sauvegarde dans les navigateurs sans sélecteur de destination.
- Publication GitHub des huit rustines autorisée par l’utilisateur le 26 août 2026. Les mentions « non publiée » ci-dessus décrivent l’état au moment de chaque validation locale ; l’ensemble est publié en une seule livraison sur la branche `main`. Les quatre fichiers non suivis sont conservés localement et exclus de la publication.

## Prochaine étape proposée

- Contrôler la version publiée sur iPad, notamment les déclenchements tactiles, « Nouveau projet » et « Ouvrir » ; contrôler « Lier le fichier de travail » dans un navigateur compatible.

## Reste à faire

- Attendre le résultat des contrôles réels avant toute nouvelle modification du programme.
